...
server_task_result_ptr server_response_reader::next(const std::function<bool()> & should_stop) {
    while (true) {
        ...
        server_task_result_ptr result = next_with_timeout(should_stop, polling_interval_seconds, timed_out);
        if (result != nullptr || !timed_out) {
            return result;
        }
    }
...
server_task_result_ptr server_response_reader::next_with_timeout(
        const std::function<bool()> & should_stop,
        int timeout,
        bool & timed_out) {
    timed_out = false;

    server_task_result_ptr result = queue_results.recv_with_timeout(id_tasks, timeout);
    if (result == nullptr) {

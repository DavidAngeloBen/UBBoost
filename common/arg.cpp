...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        ...
        "server-only: disable speculative draft contexts while prompt processing runs in the ubatch boost runtime",
        [](common_params & params) {
            params.promptprocessing_ubatchboost_spec_draft_off = true;
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}).set_env("LLAMA_ARG_PROMPTPROCESSING_UBATCHBOOST_SPEC_DRAFT_OFF"));
    add_opt(common_arg(
        {"--promptprocessing-ubatchboost-keep"},
        "server-only: reload the prompt processing ubatch boost runtime again for later idle completion requests",
        [](common_params & params) {
            params.promptprocessing_ubatchboost_keep = true;
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}).set_env("LLAMA_ARG_PROMPTPROCESSING_UBATCHBOOST_KEEP"));
    add_opt(common_arg(
        {"--tokengeneration-no-warmup"},
        "server-only: skip warmup when loading the default runtime for token generation after ubatch boost prompt processing",
        [](common_params & params) {
